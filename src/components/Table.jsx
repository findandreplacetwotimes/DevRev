...
/** Default: Due date + Owner at 160px; Name absorbs remaining width (`fitColumnWidths`). */
...
const COLUMN_WIDTHS_STORAGE_KEY = "devrev.backlog.columnWidths"
...
/** Map stored column widths to pixels that fill the viewport without horizontal scroll when possible (shrink Name first; grow Name when slack). */
function fitColumnWidths(widths, containerInnerWidth) {
  ...
  let total = out.task + out.dueDate + out.owner
...
export function Table({ className = "", rows = null }) {
  ...
  /** Narrower than column minimums — horizontal scroll only in that edge case. */
  const needsHorizontalOverflow = Boolean(
    containerWidth > 0 && containerWidth < minTableDisplay
  )
  ...
  const toggleAllRowsSelected = () => {
    ...
    setSelectedIds((prev) => {
      if (prev.size === issueRows.length) return new Set()
      return new Set(issueRows.map((issue) => issue.id))
    })
  }

  useEffect(() => () => stopResize(), [])

  /** @type {import("../lib/issuesApi").Issue[]} */
  const issueRows = Array.isArray(rows) ? rows : issues ?? []
  const allRowsSelected = issueRows.length > 0 && selectedIds.size === issueRows.length
  ...
  return (
    ...
    >
      {issues === null ? (
        <p className="border-b border-[#f2f2f3] px-[12px] py-[16px] text-[13px] text-[#939393]">Loading backlog…</p>
      ) : null}
      <div ref={tableRef} className="w-full max-w-full">
        <div className="flex w-full max-w-full border-b border-[#f2f2f3]">
          ...
          <div
            ...
            style={{
              width: displayWidths.task,
              minWidth: MIN_WIDTHS.task,
            }}
        ...
        {issueRows.map((issue) => {
          ...
          const trimmedTitle = typeof issue.title === "string" ? issue.title.trim() : ""
          const displayTitle = trimmedTitle.length ? trimmedTitle : EMPTY_ISSUE_TITLE_PLACEHOLDER

          const isRowHovered = hoveredRowId === issue.id
          const isRowChecked = selectedIds.has(issue.id)
          const showSelectControl = hasAnyRowSelected || isRowHovered

          const goToIssue = (event) => {
            const el = /** @type {HTMLElement} */ (event.target)
            if (el.closest("[data-issue-row-no-nav], button, a")) return
            if (event.shiftKey) {
            ...
            >
              ...
              >
                <div className="flex h-[48px] items-center justify-end border-b border-[#f2f2f3] py-[8px]">
                  <Selector
                    ...
                    onPress={() => toggleRowSelected(issue.id)}
                  />
                </div>
              </div>
              <div
              ...
              >
                <TableCell
                  ...
                  onToggleRowSelect={() => toggleRowSelected(issue.id)}
                  showSelector={false}
                />
              </div>
              <div
              ...
              <div data-issue-row-no-nav className="shrink-0 overflow-visible" style={ownerColStyle}>
                <TableCell
                  type="control"
                  selectorType="owner"
                  owners={OWNERS}
